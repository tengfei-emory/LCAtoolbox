...
Rcpp::List varestcpp_sltca_prob(arma::vec t0, arma::mat y,
  ...
  arma::mat Ba = zeros<mat>((num_class-1),(num_class-1));
  arma::mat Bb = zeros<mat>(num_class*num_feature*lbeta,num_class*num_feature*lbeta);
  arma::mat Bab = zeros<mat>((num_class-1),num_class*num_feature*lbeta);
  arma::mat S = zeros<mat>((num_class-1)+num_class*num_feature*lbeta,
                           (num_class-1)+num_class*num_feature*lbeta);
  
  // Rcpp::Rcout << "flag1 " << Ba << endl;
  
  for (int i = 0; i < n; ++i){
    
    arma::vec bigBa = zeros<vec>(num_class-1);
    arma::vec bigBb = zeros<vec>(num_feature*num_class*lbeta);
    
    arma::vec geex = covgee.row(i).t();
    
    // Rcpp::Rcout << "flag2 " << geex << endl;
    ...
    for (int c = 0; c < num_class; ++c){
      ...
      //alpha
      arma::vec qalpha = zeros<vec>((num_class-1));
      
      for (int k = 1; k < num_class; ++k){
        qalpha(k-1) = - p(i,k);
      }
      if (c > 0){
        qalpha(c-1) = qalpha(c-1) + 1;
      }
